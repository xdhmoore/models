...
class EvalMetricOpsVisualization(six.with_metaclass(abc.ABCMeta, object)):
  ...
  def add_images(self, images, source_id):
    ...
    if self._viz_export_dir:
      ...
      # using self._img_export_counter is thread-safe... Also, I'm not sure how
      # this will behave during train_and_eval, when eval is invoked multiple
      # times. I think it may cause the counter to be reset, which if the
      # images are evaluated in the same order could cause images from previous
      # evals to be overwritten. Or if they are not evaluated in the same order
      # and source_id is used in the filenames, it could lead to confusing
      # filenames. Perhaps all the images of a previous eval should be removed
      # before a new eval phase attempts to save images in the same folder.
  ...
  def get_estimator_eval_metric_ops(self, eval_dict):
    ...
    if type(image_tensors) in (list, tuple):
      ...
      # If self._max_examples_to_draw is 0, there will be no fetchable tf summary
